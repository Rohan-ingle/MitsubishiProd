...
MODEL_DIR = os.path.join(BASE_DIR, 'models', 'bank1')
os.makedirs(MODEL_DIR, exist_ok=True)

# Load dataset and split for bank1
df = pd.read_csv(DATA_PATH)
mid = len(df) // 2
# df_bank1 = df.iloc[:mid]
df_bank1 = df
X = df_bank1.drop('Class', axis=1)
y = df_bank1['Class']

# Train-test split (bank1 local)
...
# Save local model
joblib.dump(model, os.path.join(MODEL_DIR, 'svm_model_bank1.joblib'))
np.save(os.path.join(MODEL_DIR, 'svm_weights_bank1.npy'), model.coef_)
np.save(os.path.join(MODEL_DIR, 'svm_bias_bank1.npy'), model.intercept_)
